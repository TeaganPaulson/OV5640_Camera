...
camera_config_t config = {
    ...
    .xclk_freq_hz = 20000, // 24 MHz is what the OV5640 datasheet recommends
    .pixel_format = PIXFORMAT_JPEG,
    .frame_size = FRAMESIZE_FHD, // Reduced to HVGA (480x320) for reliability
...
void app_main() {
    ...
    while (1) {
        ...
        if (!pic) {
            vTaskDelay(pdMS_TO_TICKS(50)); // Wait for the camera to power up
            continue;
        }
